...
# Base de données PostgreSQL avec configuration avancée
resource "azurerm_postgresql_flexible_server" "fusionauth" {
  name                = "fusionauth-db"
  resource_group_name = azurerm_resource_group.fusionauth.name
  location            = azurerm_resource_group.fusionauth.location
  version            = "16"  # Dernière version stable de PostgreSQL
  
  administrator_login    = "fusionauth_admin"
  administrator_password = var.admin_password

  storage_mb = 32768
  sku_name   = "B_Standard_B2s"  # Plus puissant pour la production
...
# Container Instance pour FusionAuth
resource "azurerm_container_group" "fusionauth" {
  ...
  container {
    name   = "fusionauth"
    image  = "fusionauth/fusionauth-app:1.43.0"  # Dernière version stable
    cpu    = "2"
    memory = "4"
    ...
    environment_variables = {
      ...
      "FUSIONAUTH_APP_URL" = "https://auth.${var.domain_name}"
...
resource "azurerm_application_gateway" "fusionauth" {
  ...
  ssl_certificate {
    name     = "fusionauth-ssl"
    data     = filebase64("path/to/your/certificate.pfx")  # À remplacer par votre certificat
    password = "your-certificate-password"  # À remplacer par votre mot de passe
  }
